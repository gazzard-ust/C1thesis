...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        # DETECTION PARAMETERS - OPTIMIZED FOR PERSON DETECTION
        self.target_lost_frames = 0
        self.detection_confidence_threshold = 0.25  # Higher threshold for person detection (COCO class)
        self.min_detection_frames = 3  # Fewer frames needed for person detection
        self.current_detection_count = 0  # Track consecutive detections
        self.detection_memory_frames = 30  # Remember detection for this many frames
        self.last_known_direction = 0
        self.current_turn_speed = 0.0

        # GEOMETRIC FILTERING - DISABLED FOR PERSON DETECTION
        self.min_crack_aspect_ratio = 0.3  # Not used for person
        self.max_crack_area_ratio = 0.8  # Not used for person
        self.min_crack_size = 50  # Not used for person
        self.max_crack_size = 600  # Not used for person
        self.enable_aspect_ratio_filter = False  # Disable geometric filtering for person detection
    ...
    def initialize_models(self):
        ...
        # Person detection - "person" is a standard COCO class
        person_descriptions = [
            "person"
        ]

        self.model.set_classes(person_descriptions)
        self.get_logger().info(f"✅ YOLO-World configured with {len(person_descriptions)} person detection prompts")
...
def reset_target():
    ...
    try:
        # Reset to default classes (person for this GUI)
        node.model.set_classes(["person"])
...
def root():
    ...
    <body>
        <div class="main-container">
            ...
            <div class="top-controls">
                ...
                <div class="gas-display">
                    mAP@50: <span id="avgConfidence">37.4</span>% | Detections: <span id="detectionCount">0</span>
                </div>
    ...
    <script>
        ...
        // Search button - start looking for person
        function startSearch() {
            fetch('/set_target?object_name=person')
                .then(response => response.json())
                .then(data => {
